    ...
    public class GioHangsController : Controller
    {
        ...
        public ActionResult DatHang()
        {
            ...
            if (thanhtoan == "banking")
            {
                ...
                Session["DonHangThanhToan"] = donHang;
                Session["SanPhamThanhToan"] = strSanPham;
                return Redirect(url);
            }
            ...
            DoAn.Common.MailHeper.sendEmail("NEOTECH Computer", "Đơn Hàng #" + donHang.madonhang, contentCustomer.ToString(), taikhoan.email);
                ...
                if (checkSignature)
                {
                    if (vnp_ResponseCode == "00")
                    {
                        ...
                        if (donHang != null)
                        {
                            ...
                            donHang.thanhtoan = "Thanh toán VnPay QR";
                            db.SaveChanges();
                            ...
                            string contentCustomer = System.IO.File.ReadAllText(Server.MapPath("~/Content/Template/send2.html"));
                            contentCustomer = contentCustomer.Replace("{{madon}}", donHang.madonhang.ToString());
                            contentCustomer = contentCustomer.Replace("{{sanpham}}", Session["SanPhamThanhToan"].ToString());
                            contentCustomer = contentCustomer.Replace("{{hoten}}", khachHang.hoten);
                            contentCustomer = contentCustomer.Replace("{{ngaydat}}", donHang.ngaydat.ToString());
                            contentCustomer = contentCustomer.Replace("{{tongtien}}", donHang.tongtien.ToString());
                            contentCustomer = contentCustomer.Replace("{{diachi}}", donHang.diachi);
                            contentCustomer = contentCustomer.Replace("{{dienthoai}}", donHang.dienthoai);
                            contentCustomer = contentCustomer.Replace("{{email}}", khachHang.email);
                            DoAn.Common.MailHeper.sendEmail("NEOTECH Computer", "Đơn Hàng #" + donHang.madonhang, contentCustomer.ToString(), khachHang.email);

                            // Xóa session sau khi xử lý xong
                            Session.Remove("DonHangThanhToan");
                            Session.Remove("SanPhamThanhToan");
